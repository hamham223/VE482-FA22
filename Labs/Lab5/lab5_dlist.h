...
void dlistAppend(dlist _this, const char* key, dlistValue value);
...
void dlistSort(dlist_const _this, dlist listDst, dlistSortMethod method);

// REQUIRES: argument 'this' is not null and is a valid list
// EFFECTS: Print the content of 'this' in the required format to standard output.
//A newline must be followed after each line (including the last one).void dlistFree(dlist this);
void dlistPrint(dlist_const _this);
