...
public:
  ...
  //       dataNew.clear();
  // }
  // void swapData(Iterator &it){

  // }
  bool duplicate(KeyType key) {
    key.append("_copy");
    // dataNewMutex.lock();
  ...
  void insertDuplicate(KeyType key){
  ...
  ConstIterator end() const { return {data.cend(), this}; }
    ...
    /**
     * @brief erase “this" row by swap with the last row
     * @return nothing
     */
    void erase(const Iterator & cur){
      std::string _key_to_del = (cur.it)->key; 
      *(cur.it) = *(data.end()-1);
      keyMap[(data.end()-1)->key] = keyMap[(cur.it)->key];
      keyMap.erase(_key_to_del);
      data.pop_back();
    }

    void deleteDatum(KeyType key) {
      size_t there = (size_t)(((*this)[key])->it - data.begin());
      keyMap[data.back().key] = there;
      keyMap.erase(key);
      data[there] = std::move(data.back());
      data.pop_back();
    }
  /**
