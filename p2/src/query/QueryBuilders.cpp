...
#include "management/SwapQuery.h"
#include "management/DuplicateQuery.h"

#include "management/CopyTableQuery.h"
#include "management/TruncateTableQuery.h"

#include "management/CountQuery.h"
#include "management/DeleteQuery.h"
#include "management/SelectQuery.h"
#include "management/SumQuery.h"
#include "management/MinMaxQuery.h"
...
ManageTableQueryBuilder::tryExtractQuery(TokenizedQueryString &query) {
  ...
  if (query.token.size() == 3) {
    ...
    if (query.token.front() == "COPYTABLE")
      return std::make_unique<CopyTableQuery>(query.token[1],query.token[2]); // Not implemented
  }
  return this->nextBuilder->tryExtractQuery(query);
}
...
Query::Ptr ComplexQueryBuilder::tryExtractQuery(TokenizedQueryString &query) {
  ...
  if (operation == "DELETE")
      return std::make_unique<DeleteMulTQuery>(
          this->targetTable, this->operandToken, this->conditionToken);
  if (operation == "DUPLICATE")
    ...
    return std::make_unique<DuplicateQuery>(
          this->targetTable, this->operandToken, this->conditionToken);
  if (operation == "COUNT")
    return std::make_unique<NopQuery>(); //Does not need to be implemented
    //return std::make_unique<CountQuery>(
    //    this->targetTable, this->operandToken, this->conditionToken);
  if (operation == "SUM")
    return std::make_unique<SumMulTQuery>(
            this->targetTable, this->operandToken, this->conditionToken);
  if (operation == "MIN")
